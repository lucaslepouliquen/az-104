...
**⚠️ ORDRE CORRECT DES CMDLETS POWERSHELL - QUESTION FRÉQUENTE À L'EXAMEN**

**Question typique :** "Identify the PowerShell cmdlets required to configure recurring backups using the default backup policy. Which three cmdlets should you run in sequence?"

**Réponse Correcte - Ordre des Cmdlets :**

```powershell
# ÉTAPE 1: Enregistrer le Resource Provider Azure Backup
Register-AzResourceProvider -ProviderNamespace "Microsoft.RecoveryServices"

# ÉTAPE 2: Créer le Recovery Services Vault
New-AzRecoveryServicesVault `
  -ResourceGroupName "myRG" `
  -Name "myVault" `
  -Location "East US"

# ÉTAPE 3: Activer la protection backup sur la ressource
...
**⚠️ Explication de l'Ordre :**

1. **Register-AzResourceProvider**
   - **Premier** : OBLIGATOIRE avant toute opération Azure Backup
   - Enregistre le provider `Microsoft.RecoveryServices` dans votre subscription
   - Requis la **première fois** que vous utilisez Azure Backup dans une subscription

2. **New-AzRecoveryServicesVault**
   - **Deuxième** : Crée le conteneur pour stocker les backups
   - Le vault doit exister avant de pouvoir activer la protection

3. **Enable-AzRecoveryServicesBackupProtection**
   - **Troisième** : Active le backup sur la ressource (VM, SQL, Files, etc.)
   - Nécessite que le vault existe et soit configuré

**⚠️ ERREUR COURANTE QCM - Cmdlets Souvent Confondus :**

| Cmdlet | Usage | Quand l'utiliser ? |
|--------|-------|-------------------|
| ✅ **Register-AzResourceProvider** | Enregistrer provider | **1ère étape** - Obligatoire first time |
| ✅ **New-AzRecoveryServicesVault** | Créer vault | **2ème étape** - Créer conteneur backup |
| ✅ **Enable-AzRecoveryServicesBackupProtection** | Activer backup | **3ème étape** - Protéger ressource |
| ❌ **Backup-AzRecoveryServicesBackupItem** | Backup manuel (on-demand) | ⚠️ PAS pour configuration initiale |
| ❌ **Set-AzRecoveryServicesBackupProperty** | Modifier propriétés vault | ⚠️ Après création vault (optionnel) |

**⚠️ Point Clé pour l'Examen :**
- **Register-AzResourceProvider** : Toujours EN PREMIER
- **New-AzRecoveryServicesVault** : Avant Enable-Protection
- **Enable-AzRecoveryServicesBackupProtection** : En dernier pour activer backup
- **Backup-AzRecoveryServicesBackupItem** : ❌ N'est PAS un cmdlet de configuration (c'est pour backup on-demand)

**Création via PowerShell - Exemple Complet :**
```powershell
# ÉTAPE 1: Enregistrer Resource Provider (si première fois)
Register-AzResourceProvider -ProviderNamespace "Microsoft.RecoveryServices"

# Vérifier statut enregistrement
Get-AzResourceProvider -ProviderNamespace "Microsoft.RecoveryServices" | Select-Object ProviderNamespace, RegistrationState

# Attendre que RegistrationState = "Registered"

# Créer Resource Group
New-AzResourceGroup -Name "myRG" -Location "East US"

# ÉTAPE 2: Créer Recovery Services Vault
New-AzRecoveryServicesVault `
  -ResourceGroupName "myRG" `
  -Name "myVault" `
  -Location "East US"
...
# Configurer Storage Redundancy (optionnel mais recommandé)
Set-AzRecoveryServicesBackupProperty `
  -Vault $vault `
  -BackupStorageRedundancy GeoRedundant

# Obtenir la policy par défaut
$policy = Get-AzRecoveryServicesBackupProtectionPolicy `
  -Name "DefaultPolicy" `
  -VaultId $vault.ID

# ÉTAPE 3: Activer la protection backup
Enable-AzRecoveryServicesBackupProtection `
  -ResourceGroupName "myRG" `
  -Name "myVM" `
  -Policy $policy `
...
Daily backups are maintained for 30 days by default. This is the retention period that is specified in the default backup policy.

Daily backups are not maintained for 7 days by default. This is the minimum daily backup retention period that you can specify when you create a custom backup policy.

Daily backups are not maintained for 14 days by default. This time period is related to backups as this is the retention period for data after a VM and its restore points have been soft deleted. The VM is considered soft deleted and can be recovered at any time during that 14 day period.

Daily backups are not maintained for 9,999 days by default. This is the maximum value that you can specify for daily backup retention when you create a custom backup policy.
