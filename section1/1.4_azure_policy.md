...
6. **Audit if not exist**
7. **Modify** - Modifie les propriétés

### Effects Principaux - Détaillé

| Effet | Action | Quand utiliser |
|-------|--------|----------------|
| **Disabled** | La policy est ignoré | Testing situations, désactivation temporaire sans supprimer la policy
| **Append** | Ajoute des champs UNIQUEMENT si absents. Si présents avec valeur différente → **DENY** |  add more fields to the requested resource during creation or update. A common example is specifying allowed IPs for a storage resource. is intended for use with non-tag properties. While append can add tags to a resource during a create or update request, it's recommended to use the modify effect for tags instead. 
...
| **DeployIfNotExists** | Déploie une ressource si absente | Agents de monitoring, déploiements automatiques sur nouvelles ressources | |
