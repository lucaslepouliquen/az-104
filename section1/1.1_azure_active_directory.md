...
**Accès aux Distribution Groups :**Exchange Admin Center → Recipients → Groups → Distribution list
OU
...
** Erreur fréquente identifiée :** Syntaxe des règles dynamiques// Correct
...
**Configuration Recommandée :**Nombre de méthodes requises pour reset : 2 (minimum recommandé)
...
**Activation Password Writeback via Azure AD Connect :**# Dans l'assistant Azure AD Connect
...
**Fonctionnement Password Writeback :**Utilisateur réinitialise mot de passe dans Azure AD
    ↓
...
**Configuration SSPR via Azure Portal :**# Navigation
Azure AD → Password reset → Properties
...
**Azure AD Connect synchronise UNIQUEMENT les objets utilisateur et leurs attributs, **PAS les licences**
...
- **Eviter** de se verrouiller hors du tenant
- **Utiliser** des comptes séparés avec MFA alternatif

**3. Require Multiple Controls :**Grant Controls Options:
- Require ALL the selected controls (AND logic)
- Require ONE of the selected controls (OR logic)
**Réponses correctes QCM :**

| Question | Réponse Correcte |
|----------|----------------------|
| **"Comment bloquer accès hors du bureau ?"** | "Créer une policy avec Locations + Block access" |
| **"Conditional Access est gratuit ?"** | "Non, nécessite Azure AD Premium P1" |
| **"Peut-on utiliser distribution groups ?"** | "Non, uniquement Security Groups et M365 Groups" |
| **"MFA suffit pour appareil non compliant ?"** | "Non, utiliser 'Require compliant device'" |
...
- Activation temporaire des rôles privilégiés
- Just-in-time access pour les administrateurs
- Audit trail des élévations de privilèges