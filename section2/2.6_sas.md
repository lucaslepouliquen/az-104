...
**⚠️ Concept Clé pour AZ-104 : Les Shared Access Signatures (SAS) fournissent un accès délégué sécurisé aux ressources de stockage Azure sans partager les clés du compte**
...
**⚠️ Il existe 3 types principaux de SAS - Chacun avec des use cases spécifiques**
...
# àtape 1 : Se connecter avec Azure AD
az login

# àtape 2 : Générer User Delegation SAS
...
**⚠️ Avantages User Delegation SAS :**
...
**⚠️ Concept Clé : Les Stored Access Policies permettent de contrôler et révoquer des SAS après leur émission**
...
```bash
# àtape 1 : Créer une Stored Access Policy sur un container
...
# àtape 2 : Créer un SAS lié Ã  la policy
az storage blob generate-sas \
  --account-name mystorageaccount \
  --container-name mycontainer \
  --name myblob.txt \
...
# àtape 3 : Modifier la policy (impact immédiat sur tous les SAS)
...
# àtape 4 : Révoquer tous les SAS liés Ã  la policy
...
**⚠️ Limites des Stored Access Policies :**
...
- ⚠️ **15 minutes** : Sessions temporaires (CDN, uploads)
- ⚠️ **24 heures** : Accès quotidien
- ⚠️ **7 jours** : Maximum pour la plupart des cas
