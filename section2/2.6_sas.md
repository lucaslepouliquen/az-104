...
- **Granularité** : Accès à un blob, container, file share, queue, ou table
- **Use Case** : Délégation d'accès à des ressources spécifiques
...
```bash
# Account SAS donnant accès à Blob et File Services
...
- **Clock skew** : Azure accepte jusqu'à 15 minutes de différence d'horloge
...
# Étape 2 : Créer un SAS lié à la policy
az storage blob generate-sas \
  --account-name mystorageaccount \
  --container-name mycontainer \
  --name myblob.txt \
...
# Étape 4 : Révoquer tous les SAS liés à la policy
...
|----------|---------------------|
| **"Quel SAS est le plus sécurisé ?"** | "User Delegation SAS (Azure AD)" |
| **"Peut-on révoquer un SAS sans Stored Policy ?"** | "Non, seulement en régénérant la clé du compte" |
| **"User Delegation SAS fonctionne pour Files ?"** | "Non, Blob et ADLS Gen2 uniquement" |
| **"Combien de Stored Policies par container ?"** | "5 policies maximum" |
| **"SAS sans expiration est possible ?"** | "Non, expiration toujours obligatoire" |
