...
- **Important** : Large File Shares doit être activé à la création du compte (irréversible)
...
**Concept Clé pour AZ-104 : Chaque service de stockage Azure supporte différentes méthodes d'authentification - Comprendre quelle méthode utiliser pour quel service est ESSENTIEL pour l'examen**
...
**Etape 1 : Activer l'Authentification Identity-Based**
...
**Etape 2 : Configurer les Permissions RBAC sur le File Share**
...
**àtape 3 : Configurer les ACLs Windows (NTFS Permissions)**
...
**Etape 4 : Accès depuis les Clients**
...
**Types d'Identity-Based Access pour Azure Files :**
...
- **Avantage** : àmulation complète d'AD Domain Controller
...
**⚠️ Scénario d'Examen Critique - Authentification pour Hybrid Identities :**

**Contexte Typique d'Examen :**
> "Azure file shares doivent utiliser domain services, soit on-premises soit dans Azure, pour supporter identity-based access via SMB. Dans ce scénario, vous devez utiliser l'authentification pour hybrid identities. Les hybrid user identities sont des identités dans Active Directory Domain Services (AD DS) on-premises qui sont synchronisées avec Microsoft Entra ID en utilisant soit Microsoft Entra Connect sync ou Microsoft Entra Connect cloud sync (agent léger installé depuis Microsoft Entra admin center). Cela permet à vos utilisateurs finaux d'accéder aux Azure file shares via internet **sans nécessiter une ligne de vue vers les domain controllers** depuis les VMs hybrid-joined et Entra-joined."

**✅ SOLUTION CORRECTE : Kerberos authentication for hybrid identities**

**Pourquoi cette solution ?**
- ✅ **Supporte les hybrid identities** : Identités AD DS on-premises synchronisées vers Microsoft Entra ID
- ✅ **Pas de ligne de vue requise** : Utilisateurs peuvent accéder depuis internet sans connectivité vers domain controllers
- ✅ **Compatible hybrid-joined ET Entra-joined VMs** : Fonctionne avec les deux types de machines
- ✅ **Agent léger** : Microsoft Entra Connect cloud sync peut être installé depuis admin center

**Configuration Kerberos pour Hybrid Identities :**

```bash
# Activer Kerberos authentication for hybrid identities
az storage account update \
  --name mystorageaccount \
  --resource-group myResourceGroup \
  --enable-files-aadkerb true
...
# - Authentification Kerberos via Microsoft Entra ID
# - Support pour hybrid identities (AD DS synced to Entra ID)
# - Accès sans ligne de vue vers domain controllers
```

**Prérequis pour Hybrid Identities :**
1. Active Directory on-premises (AD DS)
2. Azure AD tenant (Microsoft Entra ID)
3. **Microsoft Entra Connect** (sync application) **OU** **Microsoft Entra Connect cloud sync** (agent léger)
4. Utilisateurs synchronisés de AD DS vers Entra ID
5. Azure File Share avec Kerberos authentication activé

**Workflow d'Authentification pour Hybrid Identity :**

```
┌─────────────────────────────────────────────────────────────┐
│                    Utilisateur Final                         │
│  - PC: Entra-joined OU Hybrid-joined                        │
│  - Identité: alice@company.com (synchronisée AD→Entra)      │
└────────────────────────┬────────────────────────────────────┘
                         │ Accès via internet
                         ▼
          ┌──────────────────────────────┐
...
PAS BESOIN de connectivité vers :
- ❌ Domain Controllers on-premises
- ❌ VPN vers réseau corporate
```

---

**❌ OPTION INCORRECTE : On-premises AD DS authentication**

**Pourquoi cette option est incorrecte ?**
- ❌ **N'active PAS l'authentification pour hybrid identities**
- ❌ **Nécessite ligne de vue** : Clients doivent avoir connectivité réseau vers domain controllers on-premises
- ❌ **Pas d'accès internet** : Utilisateurs ne peuvent PAS accéder Azure file shares via internet sans VPN/ExpressRoute
- ⚠️ **Confusion courante** : On-premises AD DS authentication fournit les méthodes pour **stocker les données de directory** et contrôler l'accès, mais ne résout pas le problème d'accès distant

**Quand utiliser On-premises AD DS authentication ?**
- ✅ Environnements **entièrement on-premises** avec connectivité réseau complète
- ✅ VMs dans Azure avec **connectivité vers domain controllers** (VPN, ExpressRoute, site-to-site)
- ✅ Pas besoin d'accès depuis internet ou sans ligne de vue

**Configuration (pour référence, mais pas pour hybrid identities sans ligne de vue) :**

```bash
# Active l'intégration avec AD DS on-premises
az storage account update \
  --name mystorageaccount \
  --resource-group myResourceGroup \
  --enable-files-adds true

# LIMITATION : Nécessite connectivité réseau vers domain controllers
```

---

**❌ OPTION INCORRECTE : Microsoft Entra Domain Services authentication**

**Pourquoi cette option est incorrecte ?**
- ❌ **N'active PAS l'authentification pour hybrid identities**
- ❌ **Nécessite ligne de vue** : Clients doivent avoir connectivité réseau vers les managed domain controllers
- ❌ **Pas d'accès internet sans ligne de vue** : Même limitation que AD DS on-premises
- ⚠️ **Confusion courante** : Microsoft Entra Domain Services (AADDS) fournit des **services de domaine managés** (domain join, group policies, LDAP, Kerberos/NTLM authentication), mais ces services sont **compatibles avec AD DS traditionnel**, pas avec l'accès distant sans ligne de vue

**Qu'est-ce que Microsoft Entra Domain Services (AADDS) ?**
- Service Azure managé qui **émule** un Active Directory Domain Controller
- Fournit : Domain join, Group Policies, LDAP, Kerberos/NTLM
- Use case : Applications cloud-native nécessitant services AD sans infrastructure on-premises

**Limitation critique pour notre scénario :**
- AADDS déploie des **domain controllers dans Azure VNet**
- Clients doivent avoir **connectivité réseau** vers cette VNet (ligne de vue requise)
- Ne permet PAS l'accès depuis internet sans VPN/VNet peering

**Architecture AADDS (pour comprendre la limitation) :**

```
┌─────────────────────────────────────────────────────────────┐
│                    Utilisateur Final                        │
│  - PC: Doit avoir connectivité vers Azure VNet              │
└────────────────────────┬────────────────────────────────────┘
                         │ VPN ou VNet connectivity required
                         ▼
          ┌──────────────────────────────┐
          │   Azure VNet                 │
          │  ┌────────────────────────┐  │
...
PROBLÈME : Ligne de vue vers managed DC requis !
```

**Quand utiliser Microsoft Entra Domain Services ?**
- ✅ Applications cloud nécessitant domain join, LDAP, Kerberos/NTLM
- ✅ Pas d'infrastructure AD on-premises
- ✅ VMs dans Azure VNet avec connectivité vers AADDS
- ❌ PAS pour accès internet sans ligne de vue

---

**❌ OPTION INCORRECTE : Active Directory (AD) Kerberos authentication for Linux clients**

**Pourquoi cette option est incorrecte ?**
- ❌ **N'active PAS l'authentification pour hybrid identities**
- ❌ **Nécessite ligne de vue** : Linux clients doivent avoir connectivité vers domain controllers
- ❌ **Spécifique à Linux** : Cette option concerne uniquement les clients Linux, pas Windows
- ⚠️ **Confusion courante** : Cette méthode permet aux **Linux clients** d'utiliser Kerberos authentication **over SMB** pour Azure Files en utilisant on-premises AD DS ou Microsoft Entra DS, mais avec les **mêmes limitations** (ligne de vue requise)

**Qu'est-ce que AD Kerberos authentication for Linux clients ?**
- Permet aux systèmes Linux de s'authentifier via Kerberos pour accéder à Azure Files SMB
- Utilise `kinit` et tickets Kerberos Linux
- Support des distributions Linux modernes

**Limitation critique :**
- Linux client doit avoir **connectivité réseau** vers domain controllers (AD DS ou AADDS)
- Pas d'accès depuis internet sans ligne de vue
- Ne résout pas le problème d'accès distant

**Configuration (pour référence, mais pas pour hybrid identities sans ligne de vue) :**

```bash
# Sur Linux client
# 1. Installer Kerberos packages
...
# 3. Obtenir ticket Kerberos
kinit alice@COMPANY.COM

# 4. Monter Azure File Share
sudo mount -t cifs //storage.file.core.windows.net/share /mnt/share \
  -o sec=krb5,username=alice@COMPANY.COM

# LIMITATION : Nécessite connectivité vers dc1.company.com
```

---

**📊 Matrice de Décision - Méthodes d'Authentification Azure Files**

| Méthode | Hybrid Identities | Accès Internet sans Ligne de Vue | Use Case Principal |
|---------|-------------------|----------------------------------|-------------------|
| **Kerberos for Hybrid Identities** | ✅ OUI | ✅ OUI | **✅ EXAMEN : Scénario hybrid avec accès distant** |
| **On-premises AD DS** | ❌ Non | ❌ Non (ligne de vue requise) | Environnements on-premises avec connectivité |
| **Microsoft Entra DS (AADDS)** | ❌ Non | ❌ Non (ligne de vue requise) | Applications cloud-native nécessitant AD services |
| **AD Kerberos for Linux** | ❌ Non | ❌ Non (ligne de vue requise) | Linux clients avec connectivité AD |

---

**⚠️ Points Clés pour l'Examen :**

1. **Hybrid Identities = AD DS on-premises synchronisées vers Microsoft Entra ID**
   - Via Microsoft Entra Connect (sync application)
   - Via Microsoft Entra Connect cloud sync (agent léger)

2. **"Ligne de vue vers domain controllers" = Connectivité réseau directe**
   - VPN, ExpressRoute, site-to-site VPN, VNet peering
   - Si l'examen dit "sans ligne de vue" ou "via internet" → **Kerberos for hybrid identities**

3. **Kerberos for hybrid identities = SEULE solution pour accès internet sans VPN**
   - Authentification via Microsoft Entra ID (cloud)
   - Pas besoin de connectivité vers domain controllers
   - Support hybrid-joined ET Entra-joined VMs

4. **Confusions courantes à éviter :**
   - ❌ "On-premises AD DS" NE supporte PAS hybrid identities sans ligne de vue
   - ❌ "Microsoft Entra DS (AADDS)" NÉCESSITE ligne de vue vers managed DCs
   - ❌ "AD Kerberos for Linux" NÉCESSITE ligne de vue, ne résout pas le problème

5. **Mots-clés d'examen indiquant Kerberos for hybrid identities :**
   - "hybrid identities"
   - "sans ligne de vue vers domain controllers"
   - "accès via internet"
   - "hybrid-joined et Entra-joined VMs"
   - "Microsoft Entra Connect sync/cloud sync"

---

**Scénario d'Examen - Identity-Based Access :**

```
Which storage data service can be configured to use identity-based access? File shares
...
**Différence Kerberos vs OAuth pour Identity-Based :**
...
**3 Types de SAS et leur Relation avec IAM :**
...
**À CONNAÎTRE PAR CŒUR pour l'Examen AZ-104 :**
...
**Scénarios d'Examen par Méthode :**

**Scénario 1 : Utilisateurs Finaux Accédant à File Shares**

```
Les employés doivent accéder à des file shares avec leurs identités Azure AD, quelle méthode utiliser ? Microsoft Entra Kerberos
...
```
VM doit lire/écrire des blobs sans clés dans le code, quelle méthode utiliser ?
Managed Identity + Azure AD RBAC
...
```
Quelle méthode utiliser pour partager un fichier avec un partenaire externe pour 7 jours
User Delegation SAS (si Blob) ou Service SAS (si File)
...
```
Quelle méthode utiliser pour rendre accessibles publiquement les Images et CSS ?
...
**Installation et Configuration - àtapes Détaillées :**
...
**Etape 2 : Créer Storage Sync Service**
...
**Etape 3 : Installer Azure File Sync Agent sur Windows Server**
...
**Etape 4 : Enregistrer le Windows Server**
...
**Vérifier l'àtat de Synchronisation :**
